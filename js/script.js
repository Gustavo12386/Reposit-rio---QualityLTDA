    ...
    $("body").on("submit", "form[name=send_contato]", function (e) {
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                $(".status").empty().html('Enviando...')
            },
            success: function (retorna) {
                if (retorna == "ok") {
                    $(".status").empty().html('<span class="ok">Mensagem enviada com sucesso.</span>');
                    // Limpa os campos do formulário
                     form[0].reset();                   

                     // Limpa a mensagem de status após um breve intervalo
                    setTimeout(function() {
                        $(".status").empty();
                    }, 3000); 
                } else {
                    $(".status").empty().html('<span class="erro">Erro ao tentar enviar.</span>');
                }
