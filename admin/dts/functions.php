...
// Site
function home() {
    $atual = (isset($_GET['url'])) ? $_GET['url'] : 'principal';
    //$permissao = array('home', 'acontece', 'a-casatorre', 'realizacoes', 'servicos', 'contato', 'erro', 'single');
    $pasta = 'pag';
...
function setUri($string) {
    $a = 'À�?ÂÃÄÅÆÇÈÉÊËÌ�?Î�?�?ÑÒÓÔÕÖØÙÚÛÜü�?ÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûýýþÿŔŕ"!@#$%&*()_-+={[}]/?;:.,\\\'<>°ºª';
...
function sendMail($assunto, $mensagem, $remetente, $nomeRemetente, $reply = NULL, $replyNome = NULL) {
    ...
    $mail->From = $remetente; //$rmt;
    $mail->FromName = $nomeRemetente;

    $mail->AddAddress($remetente);

    $mail->AddReplyTo($remetente, "RE: $assunto");
...
function uploadImage($tmp, $nome, $width, $pasta) {
    ...
    switch ($ext) {
        case 'jpg': imagejpeg($nova, $pasta . $nome, 100);
            break;
...
/* * ***************************
  FUNÇÃO GRAVAT
  PEGA AVAT DOS USU�?RIOS
 * *************************** */

function gravatar($imgdefault, $email, $s = 28, $d = 'mm', $r = 'g', $img = false, $atts = array()) {
...
function linkString($link) {
    ...
    $array1 = array(" ", "á", "à", "â", "ã", "ä", "é", "è", "ê", "ë", "í", "ì", "î", "ï", "ó", "ò", "ô", "õ", "ö", "ú", "ù", "û", "ü", "ç"
        , "�?", "À", "Â", "Ã", "Ä", "É", "È", "Ê", "Ë", "�?", "Ì", "Î", "�?", "Ó", "Ò", "Ô", "Õ", "Ö", "Ú", "Ù", "Û", "Ü", "Ç");
...
/* * ***************************
  FUNÇÃO CONTROLE DE ACESSO E GESTÃO DE ESTAT�?STICAS
 * *************************** */
